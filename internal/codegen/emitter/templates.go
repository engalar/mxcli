...
// versionTemplate emits:
//  1. VersionInfos — a map literal for diagnostic/tooling use (not used by the encoder).
//  2. PropertyVersionInfo methods — one per concrete type with version-gated properties,
//     implementing version.PropertyVersioner for zero-allocation encoder dispatch.
...
{{range .Versions}}{{if and .GoName (not .IsAlias) .HasVersionGatedProps}}
// PropertyVersionInfo implements version.PropertyVersioner for {{.GoName}}.
...
func (o *{{.GoName}}) PropertyVersionInfo(camelName string) (version.PropertyVersionInfo, bool) {
	switch camelName {
	{{- range .VersionGatedProps}}
	case "{{.Name}}":
		return version.PropertyVersionInfo{Introduced: "{{.Introduced}}", Deleted: "{{.Deleted}}"}, true
	{{- end}}
	default:
