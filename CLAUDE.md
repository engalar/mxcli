...
- [ ] **Parser regenerated** — `make grammar` run and the updated files in `mdl/grammar/parser/` committed alongside the `.g4` changes
...
Regenerate after modifying `MDLLexer.g4`, `MDLParser.g4`, or any `domains/*.g4` file: `make grammar`. The generated files in `mdl/grammar/parser/` **are committed to git** — always run `make grammar` and commit the updated parser alongside any `.g4` change. When merging two branches that both modified `.g4` files, resolve the grammar conflict first, then run `make grammar` to reset the parser files cleanly. See `docs/03-development/MDL_PARSER_ARCHITECTURE.md` for design details.
