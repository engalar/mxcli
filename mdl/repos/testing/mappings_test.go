...
func TestRecordingMappingRepository_RecordsAll(t *testing.T) {
	rec := &RecordingMappingRepository{}
	_, _ = rec.Get("a-1")
	_, _ = rec.ListByType("Mappings$ImportMapping")
	...
	if len(rec.ListedTypes) != 1 || rec.ListedTypes[0] != "Mappings$ImportMapping" {
...
func TestRecordingMappingRepository_FuncOverride(t *testing.T) {
	want := errors.New("inject")
	rec := &RecordingMappingRepository{
		CreateFunc: func(MappingCreateCall) error { return want },
	}
