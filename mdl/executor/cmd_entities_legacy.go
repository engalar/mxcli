...
func buildEventHandlersLegacy(ctx *ExecContext, defs []ast.EventHandlerDef) ([]*domainmodel.EventHandler, error) {
	if len(defs) == 0 {
		return nil, nil
	}
...
func execCreateEntityLegacy(ctx *ExecContext, s *ast.CreateEntityStmt) error {
	if !ctx.Connected() {
	...
	eventHandlers, err := buildEventHandlersLegacy(ctx, s.EventHandlers)
	if err != nil {
		return err
	}
...
func execCreateViewEntityLegacy(ctx *ExecContext, s *ast.CreateViewEntityStmt) error {
	if !ctx.Connected() {
		return mdlerrors.NewNotConnected()
	}
...
func execAlterEntityLegacy(ctx *ExecContext, s *ast.AlterEntityStmt) error {
	if !ctx.Connected() {
		return mdlerrors.NewNotConnected()
	}
	...
	case ast.AlterEntityAddEventHandler:
		...
		ehs, err := buildEventHandlersLegacy(ctx, []ast.EventHandlerDef{*s.EventHandler})
		if err != nil {
			return err
		}
...
func execDropEntityLegacy(ctx *ExecContext, s *ast.DropEntityStmt) error {
	if !ctx.Connected() {
		return mdlerrors.NewNotConnected()
	}
