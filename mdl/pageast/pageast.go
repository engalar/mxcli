...
// Package pageast converts a CreatePageStmtV3 AST into a types.PageModel.
// It lives in its own package so both the executor and the canonical/page
// layer can perform the AST→model conversion without importing each other
// (which would create an import cycle: executor imports canonical/page to
// register its codec). It depends only on the standard library, mdl/ast, and
// mdl/types — no execution context is required.
package pageast
...
// it. Properties are read from WidgetV3.Properties map; field names are matched
// case-insensitively because some callers use lowercase ("caption") while
// others use mixed case ("Caption").
func PageASTToModel(s *ast.CreatePageStmtV3, moduleName string) (*types.PageModel, error) {
	if s == nil {
		return nil, fmt.Errorf("PageASTToModel: nil statement")
	}
	...
	for _, w := range s.Widgets {
		node, err := AstWidgetToNode(w, pm.ModuleName)
		if err != nil {
...
func AstWidgetToNode(w *ast.WidgetV3, moduleName string) (*types.WidgetNode, error) {
	if w == nil {
		return nil, nil
	}
	...
	for _, child := range w.Children {
		childNode, err := AstWidgetToNode(child, moduleName)
		if err != nil {
			return nil, err
		}
		if childNode != nil {
	...
	case types.WidgetDataView:
		...
		node.Footer = nil
		for _, child := range w.Children {
			cn, err := AstWidgetToNode(child, moduleName)
			if err != nil || cn == nil {
				continue
			}
