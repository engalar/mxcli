...
# 3. 可选：集成 JS Actions / Java Actions 到实际业务流程
# （复制到剪贴板、桌面通知、相对时间）
mxcli exec academy/zh/capstone-helpdesk/参考实现/12-integrate-actions.mdl  -p MyHelpdesk/MyHelpdesk.mpr

# 4. 验证（Windows 上 Studio Pro 自动查找 mxbuild）
...
| 08-java-actions.mdl | Java Action：密码哈希/验证 + 调用微流 | 05 |
| 09-js-actions.mdl  | JS Action：剪贴板、桌面通知、相对时间 + 调用纳流 | 05 |
| 10-widget.mdl     | 自定义插件构件集成（TicketStatusBadge）| 04 |
| 11-theme.mdl      | 主题定制脚手架（SCSS 占位）| 01 |
| 12-integrate-actions.mdl | 将 08、09 的动作接入页面和微流 | 08、09、04 |
...
> 08-java-actions.mdl 和 09-js-actions.mdl 定义动作但不接入业务流程（12 做接入）。
