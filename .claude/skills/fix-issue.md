...
| CE0463 "widget definition changed" (top-level TextTemplate) | A WidgetValue's `TextTemplate` is null/document out of step with the property's editorConfig visibility (default-hidden ⇒ null, visible ⇒ ClientTemplate). NOT correlated with `ValueType.Translations`. | `modelsdk/widgets/texttemplate.go` → `normalizeTextTemplateStates` + `defaultHiddenTextTemplates` | Add the widget's default-hidden property keys to the table. Confirm with the key-based BSON oracle (`scripts/ce0463-tt-oracle.py`, joins TypePointer→PropertyKey) diffing mxcli output vs `mx update-widgets` golden — assert 0 diffs. Ratchet: `TestBuildDataGrid2_TextTemplate` |
| CE0463 on a DataGrid2 with columns / a filter widget (nested or mode-specific) | Per-COLUMN `tooltip`/`exportValue`/`dynamicText` TextTemplate is mode-dependent (visible per `showContentAs`/export config), or a filter's `attrChoice`/`attr` mode mismatches the widget definition. Blanket document/null is wrong — it is per-instance. | `mdl/backend/mpr/datagrid_builder.go` (column builders `cloneAndUpdateColumnProperties`, `buildColumnDefaultProperty`) / filter attr-mode builder | Isolate the exact driver by BSON mutation: start from `mx update-widgets` golden (0 errors), flip ONE property, re-run `mx check` to confirm causation before coding. Column/filter-mode TextTemplate needs per-instance editorConfig visibility, not a static rule. |
| CE0463 on filter widget with object-type property (e.g. `filterOptions`) | MPK parser misses nested `<property>` children when they lack a `<propertyGroup>` wrapper — `Properties[].Children` stays empty, so `GenerateFromMPK` can't build the nested ObjectType, defaulting the ValueType to something else (e.g. TextTemplate). | `modelsdk/widgets/mpk/mpk.go` → `xmlProperty` struct + `walkPropertyGroup` | Add `NestedDirectProps []xmlProperty \`xml:"properties>property"\`` to `xmlProperty`; handle direct properties (no `<propertyGroup>`) in `walkPropertyGroup` and `collectNestedProperties` by wrapping in a fake `xmlPropGroup`. |
| CE0463 on a drop-down filter (`DatagridDropdownFilter`) with a spurious `filterOptions` entry the MDL never asked for (`caption=" "`, `value=""`) | `ensureRequiredObjectLists` seeded a default WidgetObject into **optional** object lists, not just required ones. An optional list (e.g. `filterOptions`) must stay empty when no items are specified; a seeded default no longer matches the platform widget definition → CE0463. `mx update-widgets` fixes it by emptying the list. NOTE: once the MPK-nested-property fix above lands, `filterOptions` gains `NestedPropertyIDs`, which is exactly what makes this seeding path fire — the two fixes are complementary. | `mdl/backend/mpr/widget_builder.go` → `ensureRequiredObjectLists` | Skip non-required object lists entirely (`if !entry.Required { continue }`) instead of only skipping those with a nested DataSource. Diagnose with the type-faithful (mongo-driver `bson.D`, NOT py-bson which collapses int32/int64) transplant harness: from the built widget, replace only `filterOptions.Value.Objects` with an empty `[int32(2)]` and confirm `mx check` drops to 0. Ratchet: `TestEnsureRequiredObjectLists_OptionalListNotSeeded` / `_RequiredListSeeded`. |
...
| Page created by mxcli has no margin/padding; widget invisible or missing spacing in Studio Pro | DataView/DivContainer/container widgets don't set Forms$Appearance when no class/style/designprops in MDL | `mdl/executor/cmd_pages_builder_v3.go` → `applyWidgetAppearanceGen` | At start of function, always set `newDefaultAppearance()` via `SetAppearance` on any widget implementing `Appearance() element.Element` when Appearance is currently nil — input widgets already have it from `applyFormWidgetDefaults`, containers get it here |
