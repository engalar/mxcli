...
func main() {
	...
	fmt.Printf("Config: %d aliases, %d prop-key overrides, %d force-concrete, %d edge-kinds, %d id-ref-scope, %d extra props, %d extra type groups, %d binary-uuid, %d binary\n",
		...
		len(suppl.binaryUUIDProps), len(suppl.binaryProps))
...
func loadSupplements() supplements {
	...
	// Build binary_properties lookup set from slice.
